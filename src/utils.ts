...
const parseNumbers = (path_to_file: string) => {
  const commands_file = fs.readFileSync(
...
const splitFileOnBlankLine = (
  path_to_file: string,
  file_section_labels?: RegExp
) => {
  const input_file = fs.readFileSync(
    path.resolve(__dirname, path_to_file),
    "utf-8"
  );
  ...
  for (let y = coord.y - 1; y <= coord.y + 1; y++) {
    ...
    for (let x = coord.x - 1; x <= coord.x + 1; x++) {
      if (
        ...
        // skip diagonals
        (!include_diagonals && x !== coord.x && y !== coord.y) ||
